...
public class MyArticleService {
    ...
    public List<String> findByUserIdTagType(String userId, int tagType, Pageable pageable) {
        List<MyArticle> list = myArticleDao.findAllByIsValidatedEqualsAndUserIdAndTagTypeOrderByCreateTimeDesc(TAKE_EFFECT_OPEN, userId, tagType, pageable).getContent();
        return findArticleIds(list);
    }

    private List<String> findArticleIds(List<MyArticle> list) {
